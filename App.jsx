...
// ─── Design Tokens — Paleta Metamorfose ───────────────────────────────────────
const C = {
  bg:"#0A0A1A", surf:"#0D1B2A", surf2:"#0F1E2E", surf3:"#1A2744",
  border:"#1A2744", border2:"#1E3050",
  text:"#E3F2FD", muted:"#90CAF9", hint:"#2A3F60",
  primary:"#1565C0", primaryL:"#42A5F5", accent:"#64B5F6", glow:"#90CAF9",
  success:"#00E676", warning:"#FFD740", error:"#FF5252",
  // aliases mantidos para compatibilidade
  coral:"#42A5F5", teal:"#00E676", gold:"#FFD740", blue:"#64B5F6", purple:"#1565C0",
};
const G = {
  primary:"linear-gradient(135deg,#1565C0,#42A5F5)",
  hero:"linear-gradient(135deg,#1565C0 0%,#42A5F5 60%,#90CAF9 100%)",
  heroR:"linear-gradient(135deg,#90CAF9 0%,#42A5F5 50%,#1565C0 100%)",
  card:"linear-gradient(145deg,#0D1B2A,#0A0A1A)",
  glow:"linear-gradient(135deg,#1565C010,#42A5F508)",
  // aliases
  coral:  "linear-gradient(135deg,#1565C0,#42A5F5)",
  teal:   "linear-gradient(135deg,#00E676,#00BFA5)",
  gold:   "linear-gradient(135deg,#FFD740,#FF8F00)",
  purple: "linear-gradient(135deg,#1565C0,#42A5F5)",
  amber:  "linear-gradient(135deg,#1565C0,#42A5F5)",
  amberW: "linear-gradient(135deg,#1565C0 0%,#90CAF9 100%)",
};
const inp = {
  width:"100%", background:C.surf3, border:`1px solid ${C.border2}`,
  color:C.text, padding:"9px 12px", borderRadius:9,
  fontSize:13, boxSizing:"border-box", outline:"none", fontFamily:"inherit",
};
const FONT_URL = "https://fonts.googleapis.com/css2?family=Bebas+Neue&family=Anton&family=Abril+Fatface&family=Oswald:wght@600&family=Playfair+Display:ital,wght@0,700;1,400&family=Merriweather:wght@700&family=Montserrat:wght@700&family=Poppins:wght@400;600;700&family=Raleway:wght@700&family=Josefin+Sans:wght@700&family=Nunito:wght@800&family=Space+Grotesk:wght@700&family=DM+Sans:wght@700&family=Dancing+Script:wght@700&family=Pacifico&family=Lobster&family=Sacramento&family=Cormorant+Garamond:wght@700&family=Libre+Baskerville:wght@700&display=swap";
...
const OS_STATUS = { "Rascunho":C.muted,"Criando":C.gold,"Ag. aprovação":"#A0C4FF","Alteração":"#E8890C","Aprovado":"#FFD580","Agendado":"#DDD6FE","Publicado":"#10B981" };
const EMPTY_DATA = {
...
const TABS = [
  ...
  {id:"publicos",  icon:"👥", label:"Públicos"},
  {id:"conteudo",  icon:"📋", label:"Conteúdo"},
  {id:"calendario",icon:"📅", label:"Agenda"},
  {id:"os",        icon:"🗓️", label:"Programados"},
  {id:"redes",     icon:"📱", label:"Redes & WhatsApp"},
...
export default function App() {
  ...
  function Sec({title,accent,children}){ return <div style={{marginBottom:22}}><div style={{fontSize:9,fontWeight:900,letterSpacing:3,background:accent?`linear-gradient(90deg,${accent},#FFD580,#FFFFFF)`:G.hero,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",marginBottom:10,textTransform:"uppercase",display:"flex",alignItems:"center",gap:8}}><div style={{height:1,width:16,background:accent||"#F5A623",opacity:.6,flexShrink:0}} />{title}<div style={{height:1,flex:1,background:accent?`linear-gradient(90deg,${accent},transparent)`:"linear-gradient(90deg,#F5A623,transparent)",opacity:.25}} /></div><div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:"18px 20px"}}>{children}</div></div>; }
  ...
  function Toggle({val,onChange,label}){ return <div onClick={()=>onChange(!val)} style={{display:"flex",alignItems:"center",gap:9,cursor:"pointer",userSelect:"none"}}><div style={{width:38,height:20,borderRadius:10,background:val?"linear-gradient(90deg,#F5A623,#FFD580)":C.border2,transition:"background .2s",position:"relative",boxShadow:val?"0 0 10px #F5A62340":"none"}}><div style={{width:16,height:16,borderRadius:"50%",background:val?"#05060A":"#FFFFFF",position:"absolute",top:2,left:val?20:2,transition:"left .2s"}} /></div><span style={{fontSize:12,color:val?"#F5A623":C.muted}}>{label}</span></div>; }
  function InfoBox({color="#F5A623",children}){ return <div style={{background:`radial-gradient(ellipse at 0% 50%,${color}08,transparent 70%)`,border:`1px solid ${color}20`,borderRadius:10,padding:"11px 15px",marginBottom:16,fontSize:12,color:C.muted,lineHeight:1.5}}>{children}</div>; }
  ...
  function Chips({k,opts,accent}){
    const val=form[k]||[];
    return <div style={{display:"flex",flexWrap:"wrap",gap:7}}>{opts.map(([v,l])=>{const on=val.includes(v);const ac=accent||co?.color||"#F5A623";return <button key={v} onClick={()=>toggleArr(k,v)} style={{padding:"5px 14px",borderRadius:20,cursor:"pointer",fontSize:12,border:`1px solid ${on?ac+"80":C.border2}`,background:on?ac+"18":C.surf3,color:on?ac:C.muted,fontWeight:on?700:400,transition:"all .12s"}}>{l}</button>;})}</div>;
  }
  ...
  // ─── HOME ─────────────────────────────────────────────────────────────────
  if(view==="home") return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Poppins',system-ui,sans-serif"}}>
      {toast&&<div style={{position:"fixed",top:16,right:16,background:G.amber,color:"#05060A",padding:"10px 20px",borderRadius:10,fontSize:13,fontWeight:700,zIndex:9999,boxShadow:"0 4px 20px #00000060"}}>{toast.msg}</div>}
      
      {/* Hero header */}
      <div style={{background:C.surf,borderBottom:`1px solid ${C.border}`,padding:"20px 32px",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,background:"radial-gradient(ellipse at 20% 50%,#F5A62315 0%,transparent 60%),radial-gradient(ellipse at 80% 30%,#FFD58010 0%,transparent 50%)",pointerEvents:"none"}} />
        <div style={{maxWidth:960,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative"}}>
          <div style={{display:"flex",alignItems:"center",gap:14}}>
            <div style={{width:42,height:42,borderRadius:11,background:G.amber,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,boxShadow:"0 0 24px #F5A62340"}}>🤖</div>
            <div>
              <div style={{fontSize:9,letterSpacing:5,color:"#F5A623",fontWeight:700,textTransform:"uppercase"}}>Social Agent · MVP</div>
              <div style={{fontSize:22,fontWeight:700,background:G.hero,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",letterSpacing:-0.5}}>Agency Hub</div>
            </div>
          </div>
          <div style={{display:"flex",gap:24,alignItems:"center"}}>
            {[
              {label:"Empresas",val:companies.length,color:"#F5A623"},
              {label:"Cadastradas",val:Object.keys(saved).length,color:"#FFD580"},
              {label:"Produtos",val:Object.values(saved).reduce((a,d)=>a+(d.produtos?.length||0),0),color:"#FFFFFF"},
            ].map(m=><div key={m.label} style={{textAlign:"center"}}>
              <div style={{fontSize:22,fontWeight:700,color:m.color}}>{m.val}</div>
              <div style={{fontSize:10,color:C.muted,letterSpacing:.5}}>{m.label}</div>
            </div>)}
            <button onClick={()=>setNewCoForm(!newCoForm)} style={{background:G.primary,color:"#fff",border:"none",padding:"9px 20px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13,display:"flex",alignItems:"center",gap:6,boxShadow:"0 4px 20px #1565C040"}}>
              + Nova Empresa
            </button>
            <button onClick={()=>{ storage.remove("sociamind-session"); setView("login"); }} style={{background:"none",border:`1px solid ${C.border2}`,color:C.muted,padding:"9px 14px",borderRadius:10,cursor:"pointer",fontSize:12}}>Sair</button>
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:960,margin:"0 auto",padding:"28px 20px"}}>
        ...
        {/* New company form */}
        {newCoForm&&<div style={{background:G.glow,border:`1px solid #F5A62325`,borderRadius:16,padding:"22px",marginBottom:24}}>
          <div style={{fontSize:11,fontWeight:700,background:G.amber,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",letterSpacing:3,marginBottom:16,textTransform:"uppercase"}}>Nova Empresa — Cadastro rápido</div>
          <div style={{display:"grid",gridTemplateColumns:"2fr 2fr 1fr 1fr",gap:12,marginBottom:14}}>
            ...
            <F label="Cor da marca"><input type="color" value={newCo.color} onChange={e=>setNewCo(p=>({...p,color:e.target.value}))} style={{width:"100%",height:42,border:`1px solid ${C.border2}`,borderRadius:9,cursor:"pointer",background:"none",padding:3}} /></F>
          ...
          <div style={{display:"flex",gap:10}}>
            <button onClick={()=>setNewCoForm(false)} style={{background:"none",border:`1px solid ${C.border2}`,color:C.muted,padding:"8px 18px",borderRadius:9,cursor:"pointer",fontSize:13}}>Cancelar</button>
            <button onClick={addCompany} disabled={!newCo.name} style={{background:G.amber,color:"#05060A",border:"none",padding:"8px 24px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:13,opacity:newCo.name?1:.4}}>✓ Criar empresa</button>
          </div>
        </div>}
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14}}>
          {companies.map(c=>{
            ...
            return <div key={c.id} onClick={()=>openCo(c)} style={{background:G.card,border:`1px solid ${d?c.color+"30":C.border}`,borderRadius:16,padding:"18px",cursor:"pointer",transition:"all .15s",position:"relative",overflow:"hidden"}}
              onMouseEnter={e=>{e.currentTarget.style.borderColor=c.color+"70";e.currentTarget.style.transform="translateY(-2px)";}}
              onMouseLeave={e=>{e.currentTarget.style.borderColor=d?c.color+"30":C.border;e.currentTarget.style.transform="translateY(0)";}}>
              {/* Color accent bar */}
              <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:`linear-gradient(90deg,${c.color},#F5A623,#FFD580)`}} />
              {d&&<div style={{position:"absolute",top:14,right:14,fontSize:11,fontWeight:700,color:c.color,background:c.color+"15",padding:"2px 9px",borderRadius:20}}>{pct}%</div>}
              <div style={{display:"flex",gap:12,alignItems:"flex-start",marginTop:4,marginBottom:12}}>
                <div style={{width:46,height:46,borderRadius:11,background:c.color+"18",border:`1.5px solid ${c.color}40`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,overflow:"hidden",flexShrink:0}}>
                ...
                <div>
                  ...
                  <div style={{fontSize:11,color:C.muted,marginTop:1}}>{c.niche}</div>
                  {d?.slogan&&<div style={{fontSize:10,color:c.color,marginTop:2,fontStyle:"italic"}}>"{d.slogan}"</div>}
                </div>
              </div>
              {d&&<div style={{height:2,background:C.surf3,borderRadius:1,overflow:"hidden",marginBottom:10}}><div style={{height:"100%",width:`${pct}%`,background:`linear-gradient(90deg,${c.color},${c.color}90)`,transition:"width .4s"}} /></div>}
              <div style={{display:"flex",gap:8,alignItems:"center"}}>
                {prods>0&&<Badge color={c.color}>{prods} produto{prods>1?"s":""}</Badge>}
                {pubs>0&&<Badge color="#FFD580">{pubs} público{pubs>1?"s":""}</Badge>}
                {!d&&<span style={{fontSize:11,color:C.hint}}>Iniciar →</span>}
              </div>
            </div>;
          })}
        </div>
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Poppins',system-ui,sans-serif",display:"flex",flexDirection:"column"}}>
      {toast&&<div style={{position:"fixed",top:14,right:14,background:toast.type==="teal"?G.amber:"linear-gradient(135deg,#E8890C,#F5A623)",color:"#05060A",padding:"10px 18px",borderRadius:10,fontSize:13,fontWeight:700,zIndex:9999,boxShadow:"0 4px 24px #F5A62350"}}>{toast.msg}</div>}

      {/* Topbar */}
      <div style={{background:C.surf,borderBottom:`1px solid ${C.border}`,padding:"10px 22px",display:"flex",alignItems:"center",justifyContent:"space-between",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <button onClick={()=>setView("home")} style={{background:"none",border:`1px solid ${C.border2}`,color:C.muted,padding:"5px 12px",borderRadius:8,cursor:"pointer",fontSize:12}}>← Hub</button>
          ...
          <div>
            <div style={{fontSize:14,fontWeight:700,display:"flex",alignItems:"center",gap:6}}>{form.nomeFantasia||co.name}<span style={{fontSize:10,background:co.color+"15",color:co.color,padding:"2px 7px",borderRadius:20}}>{co.niche}</span></div>
            {form.slogan&&<div style={{fontSize:10,color:C.muted,fontStyle:"italic"}}>"{form.slogan}"</div>}
          </div>
        </div>
        <button onClick={save} disabled={saving} style={{background:G.amber,color:"#05060A",border:"none",padding:"8px 22px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:13,opacity:saving?.7:1,boxShadow:"0 4px 18px #F5A62330"}}>
          {saving?"Salvando…":"💾 Salvar"}
        </button>
      </div>

      {/* Tabs */}
      <div style={{background:C.surf,borderBottom:`1px solid ${C.border}`,padding:"0 22px",display:"flex",overflowX:"auto",flexShrink:0,gap:2}}>
        {TABS.map(t=><button key={t.id} onClick={()=>setTab(t.id)} style={{background:"none",border:"none",borderBottom:`2px solid ${t.id===tab?"#F5A623":"transparent"}`,padding:"10px 14px",cursor:"pointer",whiteSpace:"nowrap",color:t.id===tab?"#FFD580":C.muted,fontWeight:t.id===tab?700:400,fontSize:12,display:"flex",alignItems:"center",gap:5,transition:"color .15s",textShadow:t.id===tab?"0 0 12px #F5A62360":"none"}}>{t.icon}{t.label}</button>)}
      </div>
  ...
  function TabGerar(){
    ...
    return <>
      {/* Header */}
      <div style={{marginBottom:24,padding:"20px 22px",background:G.glow,border:`1px solid ${C.primary}20`,borderRadius:16,position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:-30,right:-30,width:120,height:120,background:`radial-gradient(circle,${C.primary}18,transparent 70%)`}} />
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:10}}>
          <div style={{width:48,height:48,borderRadius:12,background:G.primary,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,boxShadow:`0 0 24px ${C.primary}40`,flexShrink:0}}>✨</div>
          <div>
      ...
      <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:"20px",marginBottom:14}}>
        ...
        <button onClick={gerarConteudo} disabled={status==="loading" || !gForm.solicitacao.trim()}
          style={{background:status==="loading"?C.surf3:G.primary,color:"#fff",border:"none",padding:"12px 32px",borderRadius:10,cursor:status==="loading"?"default":"pointer",fontWeight:700,fontSize:14,boxShadow:status==="loading"?"none":`0 4px 24px ${C.primary}40`,display:"flex",alignItems:"center",gap:8,opacity:gForm.solicitacao.trim()?1:.5}}>
      ...
      {status==="done"&&resultado&&<div>
          ...
          <div key={idx} style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:"18px",marginBottom:12,position:"relative"}}>
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
              ...
              <button onClick={()=>salvarNoCalendario(typeof opcao==="string"?opcao:JSON.stringify(opcao), idx)}
                style={{background:G.primary,color:"#fff",border:"none",padding:"6px 16px",borderRadius:8,cursor:"pointer",fontWeight:700,fontSize:11,boxShadow:`0 2px 12px ${C.primary}40`}}>
                + Calendário
              </button>
            </div>
        ...
        <div style={{background:`${C.primary}10`,border:`1px solid ${C.primary}20`,borderRadius:12,padding:"14px 16px",fontSize:12,color:C.muted}}>
...
RETORNE APENAS JSON válido, sem texto antes ou depois, sem markdown. Estrutura:
{
  ...
  "proximosPassos": ["Ação prioritária 1", "Ação 2", "Ação 3", "Ação 4", "Ação 5"]
    ...
    return <>
      {/* Header */}
      <div style={{marginBottom:16,padding:"20px 22px",background:G.glow,border:`1px solid ${C.primary}20`,borderRadius:16,position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:-30,right:-30,width:120,height:120,background:`radial-gradient(circle,${C.primary}18,transparent 70%)`}} />
        <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:10}}>
          <div style={{width:48,height:48,borderRadius:12,background:G.primary,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,boxShadow:`0 0 24px ${C.primary}40`,flexShrink:0}}>🤖</div>
          <div>
      ...
      {temTokens
        ...
        : <div style={{background:`${C.primary}10`,border:`1px solid ${C.primary}20`,borderRadius:10,padding:"10px 14px",marginBottom:14,fontSize:12,color:C.muted,display:"flex",alignItems:"center",gap:8}}>
      ...
      <div style={{background:C.surf,border:`1px solid ${C.border}`,borderRadius:14,padding:"18px 20px",marginBottom:14}}>
        <div style={{fontSize:9,fontWeight:900,letterSpacing:3,background:G.amber,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",marginBottom:14,textTransform:"uppercase"}}>Perfis & Links da Marca</div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
          {[
        ...
        <div style={{marginTop:14,display:"flex",gap:10,alignItems:"center"}}>
          <button onClick={runScan} disabled={phase==="scanning"} style={{background:phase==="scanning"?C.surf3:G.amber,color:phase==="scanning"?C.muted:"#05060A",border:"none",padding:"11px 28px",borderRadius:10,cursor:phase==="scanning"?"default":"pointer",fontWeight:700,fontSize:14,boxShadow:phase==="scanning"?"none":"0 4px 24px #F5A62440",display:"flex",alignItems:"center",gap:8}}>
      ...
      {result&&<>
        ...
        <div style={{background:"linear-gradient(135deg,#F5A62315,#FFD58008)",border:"1px solid #F5A62330",borderRadius:14,padding:"16px 20px",marginBottom:18,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16}}>
          ...
          <button onClick={applyToForm} disabled={applying} style={{background:G.amber,color:"#05060A",border:"none",padding:"11px 26px",borderRadius:10,cursor:applying?"default":"pointer",fontWeight:700,fontSize:13,flexShrink:0,boxShadow:"0 4px 24px #F5A62440",opacity:applying?.7:1}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {result.personas.map((p,i)=>(
                <div key={i} style={{background:C.surf3,border:`1px solid ${C.border2}`,borderRadius:12,padding:"14px",position:"relative",overflow:"hidden"}}>
                  <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:G.amber}} />
        ...
        <div style={{textAlign:"center",padding:"8px 0 4px"}}>
          <button onClick={applyToForm} disabled={applying} style={{background:G.amber,color:"#05060A",border:"none",padding:"13px 40px",borderRadius:12,cursor:applying?"default":"pointer",fontWeight:700,fontSize:15,boxShadow:"0 4px 30px #F5A62450",opacity:applying?.7:1}}>
  ...
  function TabProdutos(){
    ...
    if(editing&&draft) return <>
      ...
      <div style={{display:"flex",gap:10,marginTop:4}}>
        ...
        <button onClick={saveProd} style={{background:G.amber,color:"#05060A",border:"none",padding:"9px 26px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:13,boxShadow:"0 4px 18px #F5A62330"}}>✓ Salvar produto</button>
      </div>
    </>;
    ...
    return <>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <button onClick={newProd} style={{background:G.amber,color:"#05060A",border:"none",padding:"9px 20px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13,boxShadow:"0 4px 18px #F5A62330"}}>+ Novo produto</button>
      </div>
  ...
  function TabPublicos(){
    ...
    if(editing&&draft) return <>
      ...
      <div style={{display:"flex",gap:10}}>
        <button onClick={()=>{setEditing(null);setDraft(null);}} style={{background:"none",border:`1px solid ${C.border2}`,color:C.text,padding:"9px 20px",borderRadius:9,cursor:"pointer",fontSize:13}}>Cancelar</button>
        <button onClick={savePub} style={{background:G.amber,color:"#05060A",border:"none",padding:"9px 26px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:13,boxShadow:"0 4px 18px #F5A62330"}}>✓ Salvar público</button>
      </div>
    </>;
    ...
    return <>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <button onClick={newPub} style={{background:G.amber,color:"#05060A",border:"none",padding:"9px 20px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13,boxShadow:"0 4px 18px #F5A62330"}}>+ Novo público</button>
      </div>
  ...
  function TabOS(){
    ...
    return <>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <button onClick={()=>setNova(!nova)} style={{background:G.amber,color:"#05060A",border:"none",padding:"9px 18px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13,boxShadow:"0 4px 18px #F5A62330"}}>+ Nova OS</button>
      </div>
  ...
  function TabCalendario(){
    ...
    return <>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        ...
        <button onClick={()=>setNovo(!novo)} style={{background:G.amber,color:"#05060A",border:"none",padding:"9px 18px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13,boxShadow:"0 4px 18px #F5A62330"}}>+ Adicionar</button>
      </div>
      ...
      {waP&&<div style={{background:"#25D36608",border:"1px solid #25D36635",borderRadius:14,padding:"18px",marginBottom:18}}>
        ...
        <div style={{display:"flex",gap:10,alignItems:"flex-end"}}>
          ...
          <button onClick={()=>reqChange(waP)} disabled={!altMsg} style={{background:altMsg?G.amber:"none",color:altMsg?"#05060A":"#F5A623",border:`1px solid #F5A62340`,padding:"9px 18px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:13,flexShrink:0}}>✏️ Alterar</button>
        </div>
...
function LoginScreen({ onLogin }) {
  ...
  // Credenciais locais — trocar por Supabase Auth quando disponível
  const USERS = [
  ...
  return (
    <div style={{minHeight:"100vh",background:"#0A0A1A",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Poppins',system-ui,sans-serif",padding:"20px"}}>
      {/* Background glow */}
      <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"radial-gradient(ellipse at 30% 40%,#1565C015 0%,transparent 60%),radial-gradient(ellipse at 70% 60%,#42A5F510 0%,transparent 50%)",pointerEvents:"none"}} />

      <div style={{width:"100%",maxWidth:400,position:"relative"}}>
        {/* Logo */}
        <div style={{textAlign:"center",marginBottom:32}}>
          <div style={{width:64,height:64,borderRadius:16,background:"linear-gradient(135deg,#1565C0,#42A5F5)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:32,margin:"0 auto 16px",boxShadow:"0 0 40px #1565C050"}}>🦋</div>
          <div style={{fontSize:28,fontWeight:700,background:"linear-gradient(135deg,#1565C0,#42A5F5,#90CAF9)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",backgroundClip:"text",letterSpacing:-0.5}}>SociaMind</div>
          <div style={{fontSize:12,color:"#90CAF9",marginTop:4,letterSpacing:2,textTransform:"uppercase"}}>by Metamorfose</div>
        </div>

        {/* Card */}
        <div style={{background:"#0D1B2A",border:"1px solid #1A2744",borderRadius:20,padding:"32px"}}>
          <div style={{fontSize:16,fontWeight:700,color:"#E3F2FD",marginBottom:4}}>Entrar na plataforma</div>
          <div style={{fontSize:12,color:"#90CAF9",marginBottom:24}}>Do físico ao digital — sua gestão começa aqui</div>

          <form onSubmit={handleLogin}>
            <div style={{marginBottom:16}}>
              <label style={{display:"block",fontSize:11,fontWeight:700,color:"#90CAF9",marginBottom:6,letterSpacing:.5,textTransform:"uppercase"}}>Email</label>
              <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="seu@email.com.br" autoComplete="email"
                style={{width:"100%",background:"#1A2744",border:"1px solid #1E3050",color:"#E3F2FD",padding:"11px 14px",borderRadius:10,fontSize:13,boxSizing:"border-box",outline:"none",fontFamily:"inherit"}} />
            </div>
            <div style={{marginBottom:20}}>
              <label style={{display:"block",fontSize:11,fontWeight:700,color:"#90CAF9",marginBottom:6,letterSpacing:.5,textTransform:"uppercase"}}>Senha</label>
              <div style={{position:"relative"}}>
                <input type={show?"text":"password"} value={senha} onChange={e=>setSenha(e.target.value)} placeholder="••••••••" autoComplete="current-password"
                  style={{width:"100%",background:"#1A2744",border:"1px solid #1E3050",color:"#E3F2FD",padding:"11px 40px 11px 14px",borderRadius:10,fontSize:13,boxSizing:"border-box",outline:"none",fontFamily:"inherit"}} />
                <button type="button" onClick={()=>setShow(!show)} style={{position:"absolute",right:12,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#90CAF9",cursor:"pointer",fontSize:14}}>{show?"🙈":"👁️"}</button>
              </div>
            </div>
            {erro&&<div style={{background:"#FF525215",border:"1px solid #FF525230",borderRadius:8,padding:"10px 14px",color:"#FF5252",fontSize:12,marginBottom:16}}>{erro}</div>}
            <button type="submit" disabled={!email||!senha}
              style={{width:"100%",background:"linear-gradient(135deg,#1565C0,#42A5F5)",color:"#fff",border:"none",padding:"13px",borderRadius:10,cursor:email&&senha?"pointer":"default",fontWeight:700,fontSize:14,boxShadow:"0 4px 24px #1565C050",opacity:email&&senha?1:.5,transition:"opacity .2s"}}>
              Entrar
            </button>
          </form>
        </div>

        <div style={{textAlign:"center",marginTop:16,fontSize:11,color:"#2A3F60"}}>SociaMind v2.0 · Metamorfose · 2026</div>
      </div>
    </div>
  );
}
